...
function PluginPage() {
  ...
  const licensePrice = {
    single: p.price,
    five: Math.round(p.price * 2.5),
    unlimited: Math.round(p.price * 6),
  }[license];
        ...
        <div className="mx-auto max-w-7xl px-5 lg:px-8 py-12 lg:py-16 grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2">
            ...
            <div className="mt-4 flex items-start gap-4">
              <div
                className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${p.iconColor} grid place-items-center text-3xl shadow-lg shrink-0`}
              >
                <span>{p.emoji}</span>
              </div>
              <div>
          ...
          <div className="card-surface p-6 h-fit lg:sticky lg:top-24">
            ...
            <Link
              to="/checkout"
              search={{ plugin: p.slug, license }}
              className="btn-ruby mt-5 w-full block text-center px-5 py-3 rounded-xl text-sm font-semibold"
            >
              Buy Now — ${licensePrice}
            </Link>
            <a
              href="#"
              className="mt-2 w-full block text-center px-5 py-3 rounded-xl text-sm font-semibold border border-border hover:border-primary hover:text-primary transition-colors"
            >
            ...
            <div className="mt-5 pt-5 border-t border-border flex gap-3 text-xs">
              <Link to="/docs" className="text-primary font-semibold">
                Docs
              </Link>
              <span className="text-border">|</span>
              <Link to="/support" className="text-primary font-semibold">
                Support
              </Link>
            </div>
          </div>
            ...
            {tab === "Screenshots" && (
              <div className="grid sm:grid-cols-2 gap-4">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="aspect-video rounded-xl border border-border bg-gradient-to-br from-primary-soft via-surface to-background grid place-items-center"
                  >
            ...
            {tab === "Compatibility" && (
              <div className="card-surface p-6 space-y-3">
                {[
                  ...
                  <div
                    key={k}
                    className="flex justify-between border-b border-border last:border-0 pb-2 last:pb-0"
                  >
                ...
                {[
                  {
                    ...
                    notes: [
                      ...
                      "Bug fix: edge case in license validation",
                    ],
                  },
                  {
                    v: "2.3.0",
                    d: "2026-02-10",
                    notes: ["New analytics dashboard", "Slack integration"],
                  },
                  {
                    v: "2.2.5",
                    d: "2026-01-04",
                    notes: ["Security hardening", "Translation updates"],
                  },
                ].map((c) => (
                ...
                {[
                  {
                    ...
                    a: "Yes — every release is tested with the latest WooCommerce version on day one.",
                  },
                  {
                    q: "Can I use one license on multiple sites?",
                    a: "Single Site licenses cover one production site. Use the 5-site or Unlimited license for more.",
                  },
                  {
                    q: "Is there a refund policy?",
                    a: "Absolutely. We offer a no-questions-asked 30-day money-back guarantee.",
                  },
                  {
                    q: "Do you offer customizations?",
                    a: "Yes — agencies and enterprise customers can request paid customization through support.",
                  },
                ].map((f) => (
                  <FAQ key={f.q} {...f} />
                ))}
            ...
            {tab === "Reviews" && (
              <div className="space-y-4">
                {[
                  {
                    n: "Sarah K.",
                    r: "Saved us 10+ hours per week. The admin UI is gorgeous.",
                    s: 5,
                  },
                  {
                    n: "Marco D.",
                    r: "Best plugin we've bought all year. Support replied in 2 hours.",
                    s: 5,
                  },
        ...
        <aside className="hidden lg:block">
          <div className="card-surface p-6 sticky top-24">
            ...
            <p className="mt-1.5 text-sm text-muted-foreground">
              Our support engineers reply in under 6 hours.
            </p>
            <Link
              to="/support"
              className="btn-ruby mt-4 block text-center px-4 py-2.5 rounded-xl text-sm font-semibold"
            >
              Contact Support
            </Link>
          </div>
